...
const ResumenProducto = ({ producto }) => {
  const { handleEditarCantidades, handleEliminarProducto } = useCluckin();
  ...
  return (
    <div className="shadow p-5 mb-3 flex gap-10 items-center">
      ...
      <div className="md:w-4/6">
        ...
        <p className="text-xl font-bold text-fuchsia-500 mt-2">
          Precio: {formatearDinero(producto.precio)}
        </p>

        <p className="text-sm text-gray-900 mt-2">
          Total: {formatearDinero(producto.precio * producto.cantidad)}
        </p>
      </div>
      ...
      <div>
        <button
          type="button"
          className="bg-fuchsia-700 flex gap-2 px-5 py-2 text-white rounded-md font-bold uppercase shadow-md w-full text-center"
